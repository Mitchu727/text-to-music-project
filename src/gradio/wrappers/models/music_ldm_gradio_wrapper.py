...
class MusicLDMGradioWrapper(ModelGradioWrapperInterface):
    id: str = MusicLDM.id
    ...
    def display_generation_parameters(wav_path):
        ...
        num_inference_steps = gr.Number(label="Number of inference steps", value=parameters["num_inference_steps"],
                                        interactive=False, visible=True)

        if parameters.get("negative_prompt") is not None:
            negative_prompt = gr.Textbox(label="Negative prompt", value=parameters["negative_prompt"],
                                         interactive=False, visible=True)
        else:
